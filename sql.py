...
--    AND OPB_DBDS.MAPPING_ID = WIDGET_INST.MAPPING_ID
  GROUP BY
  OPB_TARG.TARGET_ID,
  WIDGET_INST.MAPPING_ID,
  OPB_TARG.TARGET_NAME
...
JOIN WIDGET_INST on WIDGET_INST.WIDGET_TYPE = 1
AND OPB_SRC.SRC_ID = WIDGET_INST.WIDGET_ID
AND OPB_DBDS.MAPPING_ID = WIDGET_INST.MAPPING_ID
...
LEFT JOIN "INF_RP"."REP_TASK_ATTR" ON
  ...
  AND REP_TASK_ATTR.INSTANCE_ID in (
      ...
      ) THEN (select SESSION_INST_ID from SESSION_INSTANCE)
...
--  SRC_.INSTANCE_ID as SOURCE_INSTANCE_ID,
--  TGT_.INSTANCE_ID as TARGET_INSTANCE_ID,
  ELSE SRC_.OWNERNAME || '.' || SRC_.SOURCE_NAME END as SOURCE_TABLE,
  TGT_.TARGET_NAME as TARGET_TABLE
...
WHERE 1=1
--AND REP_WORKFLOWS.SUBJECT_ID = 92
--AND OPB_SESSION.SESSION_ID = 8400
